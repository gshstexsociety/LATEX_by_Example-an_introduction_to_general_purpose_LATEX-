...
\begin{document}
	...
	\begin{tabularx}{\textwidth\onehalfspacing}{|X|X|}
		...
		\hline
	\end{tabularx}\newline\newline
	만약 두번째 분단이 텍스트만 포함 하고 있다면 수식 환경안에 \textbf{decases*}환경을 사용하면 된다.\newline
	
	\begin{tabularx}{\textwidth\onehalfspacing}{|X|X|}
		\hline
		f(x)= \textbackslash begin\{dcases*\}
		
		\ \ \ \ \ \ 1 \& when \$x\$ is even\textbackslash\textbackslash
		
		\ \ \ \ \ \ 0 \& when \$x\$ is odd
		
		\textbackslash end\{dcases*\}
		&\[f(x)=\begin{dcases*}
		content...
		\end{dcases*}\]
	\end{tabularx}
